...
particles to obtain $\bs\Psi(\bs q)$. According to this we estimate the linear density
...
In section \ref{ss.reco}, we estimate the linear density field from the
displacement field $\bs\Psi(\bs q)$ from simulations.
Potential reconstruction algorithms are
based on an estimated displacement field $\tilde{\bs \Psi}(\bs q)$
instead of $\bs\Psi(\bs q)$. In the following sections we
simply call the estimation of linear density field
from $\tilde{\bs \Psi}(\bs q)$ ``reconstruction''.
...
reconstructed ($R$), or noise ($n$) density 
