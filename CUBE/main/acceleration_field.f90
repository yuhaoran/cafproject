subroutine acceleration_field
  ...
  open(20,file=output_name('afield'),status='replace',access='stream')
  afield=0
  ipp=0
  cum=cumsum6(rhoc)
  !nthread=1
  ...
  do itx=1,nnt
    ...
    do i=1,nt ! loop over coarse cell
      ...
      do l=1,np ! loop over particle
        ...
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx1(1),idx1(2),idx1(3))*a_mid*dt/6/pi*dx1(1)*dx1(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx2(1),idx1(2),idx1(3))*a_mid*dt/6/pi*dx2(1)*dx1(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx1(1),idx2(2),idx1(3))*a_mid*dt/6/pi*dx1(1)*dx2(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx1(1),idx1(2),idx2(3))*a_mid*dt/6/pi*dx1(1)*dx1(2)*dx2(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx1(1),idx2(2),idx2(3))*a_mid*dt/6/pi*dx1(1)*dx2(2)*dx2(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx2(1),idx1(2),idx2(3))*a_mid*dt/6/pi*dx2(1)*dx1(2)*dx2(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx2(1),idx2(2),idx1(3))*a_mid*dt/6/pi*dx2(1)*dx2(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_f(:,idx2(1),idx2(2),idx2(3))*a_mid*dt/6/pi*dx2(1)*dx2(2)*dx2(3)
  ...
  do itx=1,nnt ! loop over tiles
    ...
    do i=1,nt
      ...
      do l=1,np ! loop over particle
        ...
        dx2(:)=1-dx1(:)
        !vreal=tan(pi*real(vp(:,ip))/real(nvbin-1))/(sqrt(pi/2)/(sigma_vi*vrel_boost))
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx1(1),idx1(2),idx1(3))*a_mid*dt/6/pi*dx1(1)*dx1(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx2(1),idx1(2),idx1(3))*a_mid*dt/6/pi*dx2(1)*dx1(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx1(1),idx2(2),idx1(3))*a_mid*dt/6/pi*dx1(1)*dx2(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx1(1),idx1(2),idx2(3))*a_mid*dt/6/pi*dx1(1)*dx1(2)*dx2(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx1(1),idx2(2),idx2(3))*a_mid*dt/6/pi*dx1(1)*dx2(2)*dx2(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx2(1),idx1(2),idx2(3))*a_mid*dt/6/pi*dx2(1)*dx1(2)*dx2(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx2(1),idx2(2),idx1(3))*a_mid*dt/6/pi*dx2(1)*dx2(2)*dx1(3)
        afield(:,ipp)=afield(:,ipp)+force_c(:,idx2(1),idx2(2),idx2(3))*a_mid*dt/6/pi*dx2(1)*dx2(2)*dx2(3)
