...
At a sufficient high initial redshift $z_i$, we firstly generate a linear density fluctuation $\delta_L$ by multiplying a Gaussian random field with the transfer function $T(k)$ in Fourier space. Then we solve for a curl-less displacement field to displace particles from their initial regular positions with Zel'dovich approximation (ZA) \citep{1970A&A.....5...84Z}. We solve the potential $\Phi$ of this displacement field by Poisson equation, and then the particles are generated and displaced by locally differentiating $\Phi$ in real space. At the same time the particle is assigned with a initial velocity. Before the particles are generated, a global Pencil FFTs are applied on the fine mesh to transform the initial random Gaussian field into the final displacement potential $\Phi$. The initial condition generator is applied only once, and in principle, at some costs of speed, one can use only one scalar field on the fine mesh, and FFTs can be done in-place. If the number of particles equals the number of fine grids, then the memory consumption is dominated by the single precision fine mesh field, or 4 bpp.
