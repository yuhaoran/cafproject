...
Here we present the algorithm and accuracy of a new parallel, memory-lite, Particle-Mesh
based $N$-body code, where each particle can occupy as low as 6-byte memory. This 
...
When $N$ is large, the brute force pairwise particle-particle (PP) force brings unaffordable $o(N^2)$ computations. The workload can be alleviated by many algorithms, for example, the ``tree'' algorithms like {\tt GADGET} \citep{2001NewA....6...79S,2005MNRAS.364.1105S} and its simulation ``Millennium'' \citep{2005Natur.435..629S,2012MNRAS.426.2046A}, {\tt TPM} \citep{1995ApJS...98..355X} and {\tt GOTPM} \citep{2004NewA....9..111D}, adaptive grid algorithm including {\tt HYDRA} \citep{1995ApJ...452..797C} and {\tt RAMSES} \citep{2010ascl.soft11007T}, as well as mesh-refined codes \citep{1991ApJ...368L..23C} and moving adaptive particle-mesh codes \citep{1995ApJS..100..269P}. Among these, the standard particle-mesh (PM) algorithm \citep{1988csup.book.....H} is most memory and computational efficient if we focus on large cosmological scales. The load balancing problem is minimized as the matter distribution is rather homogeneous, and the speed benefits from the fast Fourier transform (FFT) libraries, such as {\tt FFTW3} \citep{Frigo05thedesign}. Based on PM, the parallel code {\tt PMFAST} \citep{2005NewA...10..393M} uses a two-level PM algorithm to reduce the global communication and memory. After subsequent developments, {\tt CUBEP3M} \citep{2013MNRAS.436..540H} uses cubic spatial decomposition, and adds in-cell or adjacent-cell PP force and many other features.

With the fast development of parallel 
...
$N$-body simulations use considerable amount of memory, because the phase space 
