...
We present a new $N$-body simulation code {\tt CUBE}, using as low as 6 byte per particle 
(here after we use ``bpp'' referring to ``byte per particle''). We show that it gives 
accurate results in cosmological LSS simulations. The algorithm is presented in \S 2, and 
a comparison between this method and traditional method is shown in \S 3. Discussions and 
...
example of a memory-lite parallel $N$-body code, can use as low as 40 bpp in 
...
6 bpp, described as following subsections.

\subsection{Particle position storage}
...
\end{equation}
where $[\ ]$ is the operator to take the integer part. $n_\chi\in\{1,2\}$ is the number of bytes used for each integer, $x_d$ and $\chi_d$ are floating and integer version of the coordinate. Similarly, in section \ref{s.velocity}, the velocity counterpart of them are $n_\nu=1,2$, $v_d$ and $\nu_d$. Position resolution for $n_\chi$-byte integer, ``x$n_\chi$'', is $2^{-8n_\chi}L/N_c$.
...
\begin{eqnarray}\label{eq.vdisp}
\sigma_v(a,k) = \frac{1}{3}  (a H f D)^2 \int_0^k d^3q \frac{P_{L}(q)}{q^2},
\end{eqnarray}
...
\begin{eqnarray}\label{eq.vconvert}
...
\end{equation*}
where ${\tt nex}={\tt nt}+2{\tt ncb}$ covers the buffer region on both sides, {\tt nnt} is the tile dimemsions, and {\tt nn} is the image {\it codimensions}\footnote{Coarray Fortran concept. Codimensions can do communications between images.}. Particles' phase coordinates {\tt xp} and {\tt vp} are copied between tiles and images, when necessary. Figure \ref{fig.tile} shows the spacial decomposition in a 2-dimensional analogy, with ${\tt nn=2}$ and ${\tt nnt=2}$.
\begin{figure}
\centering
...
\subsection{Code overview}\label{ss.code}
